  ...
  Zombie.prototype.takeDamage = function (damage, pos, other) {
    if (this.health > 0) {
      ...
      if (this.health <= 0) {
        ...
        this.z -= 90; // always underfoot (starts between 100 and 200)
        ...
        if (other) {
          ...
          if (other.isFirearm) {
            Game.reporter.reportZombieDeath('shot ' + inhead + 'by ' + other.description);
          } else if (other.clazz === 'BaseballBat') {
            Game.reporter.reportZombieDeath('smashed ' + inhead + 'with a baseball bat');
          } else if (other.isCar) {
            var runaway = (other.driver) ? '' : 'runaway ';
            Game.reporter.reportZombieDeath('run over by a ' + runaway + other.color + ' ' + other.clazz);
          }
        } else if (damage === 999) {
          Game.reporter.reportZombieDeath('utterly destroyed by supernatural forces (cheater!)');
        } else {
          Game.reporter.reportZombieDeath('killed by unknown causes!');
        }
      }
    }
  };
