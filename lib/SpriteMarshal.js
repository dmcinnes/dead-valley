...
define(['Console'], function (Console) {
  ...
  var marshal = function (spriteString, callback) {
    ...
    var complete = new Complete(function () {
      callback(model, sprite);
    });

    // load the class specified in the sprite string
    require(['models/'+values.clazz, 'sprites/'+values.clazz+'Sprite'], function (NewModel, NewSprite) {
      model  = new NewModel(values.type);
      setValues(model, values, complete);
      sprite = new NewSprite(model);
      complete.done();
    }, function (err) {
      Console.log("Sprite Marshal Error on " + values.clazz, err, err.stack);
      complete.done();
    });
  };
