...
define(['AssetManager',
  ...
  var Game = {
    ...
    startPosition: new Vector(1282, 1994, true),
    ...
    runSprites: function (delta) {
      if (this.map) {
        ...
        for (k = 0; k < speculativeCollisionIterations; k++) {
  ...
  Game.events.subscribe("stop game", function () {
    // so we don't save the dude after the game is over
    Game.cleanupCurrentDude();
  });
