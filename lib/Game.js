  ...
  var Game = {
    ...
    runModels: function (delta) {
      if (this.map) {
        ...
        for (i = 0; i < modelCount; i++) {
          ...
          // update timeouts
          if (sprite.updateTimeouts) {
            sprite.updateTimeouts(delta);
          }
    ...
    renderSprites: function (delta) {
      if (this.map) {
        ...
        for (var i = 0; i < spriteCount; i++) {
          ...
          if (sprite.model && sprite.model.visible) {
    ...
    newGame: function (time) {
      ...
      Game.map.loadAllMapTiles({nw:'blank', ne:'blank', sw:'blank', se:'blank'}, function () {
        ...
        require(['DudeModel', 'DudeSprite'], function (DudeModel, DudeSprite) {
          var dude   = new DudeModel();
