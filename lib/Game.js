...
define(['AssetManager',
  ...
  var speculativeCollisionIterations = 2;

  var gameStates = {
    start: function (delta) {
      Game.isOver = false;
      GameTime.setTargetTime(Game.timeLimit ? Game.targetTime : null);
  ...
  var Game = {
        ...
        for (var k = 0; k < speculativeCollisionIterations; k++) {
          ...
          for (j = 0; j < 3; j++) {
            for (i = 0; i < contactListLength; i++) {
              ...
              if (!Collidable.runRigidBodyRectifier(contact)) {
    ...
    addSprite: function (sprite) {
      ...
      // make sure cars are not overlapping something
      if (sprite.isCar) {
	var contactList = [];
	sprite.updateGrid();
	// check for collisions ignoring the collision range
	sprite.checkForCollisionsWithNearbyObjects(contactList, true);

	var length = contactList.length;
	for (var i = 0; i < length; i++) {
	  // move the car out
	  sprite.pos.translate(contactList[i].normal);
	  // console.log(sprite.name, sprite.pos.toString(), contactList[i].normal.toString());
	}
      }
