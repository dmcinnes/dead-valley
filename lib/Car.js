...
define(["Game",
  ...
  Car.prototype.setThrottle = function (throttle) {
    ...
    if (this.health > 0 && this.currentFuel > 0) {
      if (!this.accelerating) {
        Audio.engine2.accelerate.play(function () {
          Audio.engine2.speed.loop();
        });
        Audio.engine2.slow.stop();
        Audio.engine2.idle.stop();
      }
      // front wheel drive
  ...
  Car.prototype.preMove = function (delta) {
    ...
    if (this.driver) {
      ...
      if (this.accelerating && !nowAccelerating) {
        Audio.engine2.slow.play(function () {
          Audio.engine2.idle.loop();
        });
        Audio.engine2.accelerate.stop();
        Audio.engine2.speed.stop();
      }
      ...
      if (this.health <= 0) {
        ...
        if (other) {
          if (other.isFirearm) {
            ...
            reason = 'collided with a ' + runaway + color + ' ' + other.name;
  ...
  Car.prototype.drop = function (event, draggable) {
  };
