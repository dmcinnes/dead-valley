...
define(["Game", "Console", "Matrix", "Vector", "EventMachine", "SpriteMarshal", "sprite-info"],
  ...
  Sprite.prototype.init = function (name) {
    ...
    this.clazz = name.replace(/\d+$/, ''); // numbers at the end denote types
    ...
    this.pos = new Vector(0, 0, true);
    this.pos.rot = 0;

    this.vel = new Vector(0, 0, true);
    this.vel.rot = 0;

    this.acc = new Vector(0, 0, true);
    this.acc.rot = 0;
  ...
  Sprite.prototype.createNode = function (layers, z) {
    var node = $('<div/>');
  ...
  Sprite.prototype.inCollideRange = function (x, y) {
    ...
    if (this.layers) {
      ...
      for (var i = length-1; i >= 0; i--) {
        ...
        if (index >= 0) {
          ...
          position.push(left + 'px ' + top + 'px');
        }
      }
      this.node[0].style.backgroundPosition = position.join(',');
    }
  };
