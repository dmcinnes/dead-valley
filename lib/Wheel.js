...
define(["Vector", "Matrix"], function (Vector, Matrix) {
  ...
  //foward vector
  var forwardVector  = new Vector(0, -1, true);
  var sideVector     = new Vector(1,  0, true);

  var Wheel = function (posx, posy, radius) {
    this.position      = new Vector(posx, posy, true);
    this.radius        = radius;

    this.speed         = 0.0;

    this.responseForce = new Vector(0, 0, true);
    this.forwardAxis   = new Vector(0, 0, true);
    this.sideAxis      = new Vector(0, 0, true);
    this.torque        = 0.0;
  ...
  Wheel.prototype.calculateForce = function (relativeGroundSpeed, delta) {
    ...
    this.responseForce.set(sideVel.multiply(-40.0).subtract(forwardVel));
