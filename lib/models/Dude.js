...
define(["Game",
  ...
  // fire the active firearm if we have one
  Dude.prototype.fireWeapon = function () {
    if (this.activeFirearm()) {

      var firearm = this.hands.weapon();
      if (firearm && this.aimPoint) {
        var muzzle = this.aimPoint.subtract(this.pos).normalize().scale(5);

        if (firearm.fire(this.pos.add(muzzle), this.aimPoint, this.direction)) {
          this.firing = true;
        }
      }
    }
