...
define(["Vector"], function (Vector) {
  ...
  var checkForCollisionsWithNearbyObjects = function (contactList, ignoreCollideRange) {
    ...
    for (var i = 0; i < len; i++) {
      ...
      while (ref) {
	if (ref.collidable) {
	  result = this.checkCollision(ref, ignoreCollideRange);
  ...
  // checkCollision
  var checkCollision = function (other, ignoreCollideRange) {
    var normals, minDepth, nl, we, they, left, right, depth,
        minPoint, normalIndex;

    if (this === other ||
        !(other.collideRange || ignoreCollideRange) ||
        (this.ignores &&
