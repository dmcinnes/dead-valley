define(['Game', 'hud/Overlay'], function (Game, Overlay) {
  ...
  var textScreenToggler = function ($toToggle, direction) {
    ...
    return function () {
      if (!sliding) {
        ...
        if ($screen.is(':visible')) {
          $toToggle.css('top', fro * Game.GameHeight).show();
          $screen.animate({top:to * Game.GameHeight}, {duration:slideTime, queue:false, complete:function () {
            $screen.hide();
            sliding = false;
          }});
          $toToggle.animate({top:0}, {duration:slideTime, queue:false});
        } else {
          $screen.show();
          $screen.animate({top:0}, {duration:slideTime, queue:false});
          $toToggle.animate({top:fro * Game.GameHeight}, {duration:slideTime, queue:false, complete:function () {
            $toToggle.hide();
            sliding = false;
          }});
        }
  ...
  var toggleHelp  = textScreenToggler($help, 'down');
  var toggleAbout = textScreenToggler($about, 'up');
