...
define(['Game',
  ...
  // pick up inventory sprites when we click on them
  $('#sprites').on('click', '.inventory-sprite', function (e) {
    var id = $(e.target).data('sprite-id');
    var sprite = Game.getSpriteByID(id);
    if (sprite) {
      dudeInventory.restartDrag(sprite.item, null, e);
      sprite.die();
      e.stopPropagation();
    }
  });
