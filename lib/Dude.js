...
define(["Game",
  ...
  var Dude = function () {
    ...
    this.aimingArmNode[0].style['-moz-transform-origin'] = ARM_OFFSET_X + 'px ' + ARM_OFFSET_Y + 'px';
  ...
  Dude.prototype.renderToContext = function (context) {
  ...
  Dude.prototype.finalizeLayers = function () {
    ...
    if (this.imageData && (this.takingDamage || Sky.isDark())) {
  ...
  Dude.prototype.distanceFromOrigin = function () {
    ...
    return Math.round(sprite.pos.magnitude() / 1584);
  };
  ...
  Dude.prototype.setupEventHandlers = function () {
    ...
    Game.events.subscribe('before start', function () {
      ...
      if (self.inside) {
        self.updateGrid(); // otherwise currentNode is null
        if (self.currentNode.entrance) {
          self.enterBuilding(self.currentNode.entrance);
        } else {
          self.inside = false;
        }
      }

    }).subscribe('dude enter/exit', function () {
      self.enterOrExit();
      ...
      if (self.alive()) {
        ...
        } else if (!self.inside && !self.driving) {
    ...
    Sky.subscribe('sunrise', function () {
      if (self.driving) {
        self.driving.headlightsOn = false;
      }
    }).subscribe('sunset', function () {
      if (self.driving) {
        self.driving.headlightsOn = true;
      }
    });
  };
