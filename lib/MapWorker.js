...
require(['TileMarshal', 'section-list', 'car-list', 'inventory-list', 'Vector'], function (TileMarshal, section_list, car_list, inventory_list, Vector) {
  ...
  var seedShrubs = function (tiles, width, scale) {
    var x, y, i, count;
  ...
  var seedCars = function (tiles, scale) {
    ...
    if (scale > 0 &&                  // never have cars on the first seciton
        Math.random() < 0.5 &&        // 50% chance of cars
        tiles.roadTiles.length > 0) { // section must have roads

      carCount = Math.round(tiles.roadTiles.length * 0.05);
      var roadTiles = tiles.roadTiles;

      for (var i = 0; i < carCount; i++) {
        var randomRoadTile = Math.round(Math.random() * (roadTiles.length - 1));
        var tileIndex = roadTiles[randomRoadTile];
  ...
  var addCar = function (tile, tiles, scale) {
    ...
    var pos = new Vector(tile.x + Math.random() * 60,
    ...
    var car = {
      ...
      zombies: (Math.random() < scale/10) ? 1 : 0,
  ...
  var cloneSection = function (section) {
    ...
    tiles.roadTiles = section.roadTiles;
    return tiles;
  };
  ...
  onmessage = function (e) {
    ...
    seedShrubs(tiles, config.width, scale);
