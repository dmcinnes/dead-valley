...
require(['TileMarshal', 'section-list', 'car-list', 'inventory-list', 'Vector'], function (TileMarshal, section_list, car_list, inventory_list, Vector) {
  ...
  END_GAME      = 52; // last ten sections should be hellish
  ...
  var fillBlankTiles = function (tiles, width, distance) {
    var x, y, i, count;
    var total = tiles.length;
    var carCount = 0;

    var addCars = distance > END_GAME ||   // Always have cars at the end
                  (Math.random() < 0.5) && // 50% chance of cars
                  distance > 0;            // never have cars on the first seciton
    ...
    for (i = 0; i < total; i++) {
      ...
      if (tile.tileOffset === 0) {
        ...
        if (Math.random() < 0.05) {
          x = (i % width) * 60;
          y = Math.floor(i / width) * 60;
          addCar(x, y, tile, tiles, distance);
          carCount++;
        }
  ...
  var addCar = function (x, y, tile, tiles, distance) {
    ...
    var carColor = carType.colors[Math.floor(Math.random() * carType.colors.length)];
    ...
    var car = {
      ...
      zombies: (Math.random() < distance/500) ? 1 : 0,
  ...
  var seedBuildings = function (buildings, distance) {
    var zombieChance = distance / 400;
    if (distance > END_GAME) {
      zombieChance = 0.5;
    }
    ...
    return _.map(buildings, function (building) {
      ...
      if (building.entrances.length &&
        ...
        building.zombies = Math.round(distance / 400) + 1;
      }
  ...
  // TODO merge this with fillBlankTiles do we don't have to hunt
  // around for road tiles
  // better yet, fillBlankTiles should provide a list of road tiles
  ...
  onmessage = function (e) {
    ...
    var carCount = fillBlankTiles(tiles, config.width, distance);

    tiles.buildings = seedBuildings(tiles.buildings, distance);
