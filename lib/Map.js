...
define(["Vector", "Game", "GridNode", "World", "Progress", "Building", "BuildingMarshal", "Console"],
  ...
  var updateScreenOffset = function () {
    ...
    console.log(screenOffset.left, screenOffset.top);
  };
  ...
  var Map = function (gridWidth, gridHeight) {
    ...
    var mapWorker = new Worker("lib/MapWorker.js");
    ...
    this.loadStartMapTiles = function (tiles, finish) {
      if (!tiles) {
        tiles = {};
      }
      ...
      Progress.run(function (callback) {
                     self.loadMapTiles(chunks.nw, 'nw', {name:tiles.nw}, callback);
                   }, function (callback) {
                     self.loadMapTiles(chunks.sw, 'sw', {name:tiles.sw}, callback);
                   }, function (callback) {
                     self.loadMapTiles(chunks.ne, 'ne', {name:tiles.ne}, callback);
                   }, function (callback) {
                     self.loadMapTiles(chunks.se, 'se', {name:tiles.se}, callback);
                   }, function () {
                     if (finish) {
                       finish();
                     }
    ...
    this.render = function (delta) {
      if (this.needsRender) {
        ...
        while (i) {
          ...
          nodeOffset =  imageData[offset] +
                       (imageData[offset+1] << 8);
          gridX = ((i % imageWidth) + startX) * Game.gridSize - this.submapOffsetX;
          gridY = (Math.floor(i / imageWidth) + startY) * Game.gridSize - this.submapOffsetY;
